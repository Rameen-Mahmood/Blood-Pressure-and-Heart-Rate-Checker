...
float GetFromSensor()
{
  ...
  float  p_max = 300; //maximum pressure according to the datasheet for 0300YG
...
float first_peak_pressure; // pressure of the first oscillation peak to find systolic pressure
...
//function to display the pressure level and changes by using our microcontroller's neopixels
void display_pressure(float pressure){ 
  // function for displaying pressure increase/decrease on neopixels in increments of 10
...
void loop() {
  ...
  if (rate > 0.6){ //if the rate is greater than 6 mmHg/sec, the user is going too fast
    Serial.println("Release rate too fast");
  }
  else if (rate < 0.2 && rate > 0){ //if the rate is below 2 mmHg/sec, the user is going too fast
    Serial.println("Release rate too slow");
  }
  else if (rate < 0){ //less than 0 because we calculate initial - final 
  ...
  if (reached_160 == 1 and curr_pressure < 160){ // once the pressure gets up to 160, wait till it goes below 160 to look for oscillations
    if (slope > 0 && prev_slope < 0){ // if the current slope positive, and previous slope was negative --> local minimum
      ...
      if (curr_pressure < 120 && curr_pressure > 60){ // while the pressure is between 120 and 60, start counting # of oscillations
         max_counter = max_counter + 1; // increment the # of oscillations (used for heart rate calculation) -- this represents the number of beats
      }
  ...
  if (time_counter == 150){ // the timer counts up to 15 seconds (our delay is 100ms), based upon standard regulation of time to count beats to calculate heart rate 
    if (curr_pressure < 120 && curr_pressure > 60){ 
      ...
      max_counter = 0; // reset the oscillation counter (which represent beats) 
    }
  ...
  //As part of the user interface, print out systolic, diastolic, and heart rate for user to see 
